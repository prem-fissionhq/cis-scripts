...
if [ "$PROFILE" = "Level 1" ] || [ "$PROFILE" = "Level 2" ]; then
  ...
  echo
  echo \*\*\*\* Check\ That\ Reserved\ UIDs\ Are\ Assigned\ to\ System\ Accounts
...
if [ "$PROFILE" = "Level 2" ]; then
  ...
  # Ensure SELinux is not disabled in /boot/grub2/grub.cfg
  echo
  echo \*\*\*\* Ensure\ SELinux\ is\ not\ disabled\ in\ /boot/grub2/grub.cfg
